...
public class ZipUtils {
    ...
    // 压缩文件
    public static File compressFile(String path){
        try {
            ...
            if(index==-1){
                ...
                pathname= path.replace(path.substring(index,path.length()),".zip");
            }
     ...
     * @throws RuntimeException 压缩失败会抛出运行时异常
     */
    private void toZip(List<File> srcFiles , OutputStream out)throws RuntimeException {
        long start = System.currentTimeMillis();
        ZipOutputStream zos = null ;
        try {
            zos = new ZipOutputStream(out);
